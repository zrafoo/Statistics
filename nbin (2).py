...
for x in slimnbinch:
    while slimnbinch.count(x)!=1:
        slimnbinch.remove(x)
slima=a1000[0]#qqqqqqqqqqqqqqqqqqqqq
slimaemp=emp_func(0,a1000)#qqqqqqqqqqqqqqqqqqqqq
for x in slima:
    ...
    for j in range(len(slima)):
        if slima[j]==slimnbin[i]:
            if abs(slimnbinch[i]-slimaemp[j])>max:
                ...
                break
            break
f=open('add1000.txt','w') #qqqqqqqqqqqqqqqqqqqqq
f.write('Максимальная разница эмп. функции при n=5 и функции распределения - ')
f.write(str(max))
f.write('\n')
f.close()
